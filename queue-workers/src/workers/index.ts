...
export const ingestionWorker = new Worker(
  ...
  async (job) => {
    ...
    // Stage tracking is done inside FastAPI scheduler
    // This worker handles any additional ingestion-triggered logic
...
export const commentaryWorker = new Worker(
  ...
  async (job) => {
    ...
    const { event_id } = job.data;
    // Commentary is generated by FastAPI scheduler with debounce
    // This worker handles additional triggering if needed
    try {
      await axios.post(`${BACKEND_URL}/internal/commentary/${event_id}`);
    } catch (e) {
      // Backend handles this directly
    }
  ...
  },
  {
    connection: redisConnection,
    concurrency: 2,
    limiter: { max: 10, duration: 60000 }, // 10/min rate limit
  }
);
...
export const analysisWorker = new Worker(
  ...
  },
  {
    connection: redisConnection,
    concurrency: 2,
    limiter: { max: 12, duration: 60000 }, // 12/min (Gemini limit: 15/min)
  }
);
...
export const alertWorker = new Worker(
  ...
  async (job) => {
    ...
    const { event_id } = job.data;
    // Alert evaluation happens in FastAPI scheduler
